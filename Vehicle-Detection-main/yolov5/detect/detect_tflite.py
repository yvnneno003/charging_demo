...
class DetectTFLite:
    ...
    def detect(self, image=None, image_path=None):
        ...
        try:
            ...
            if self._model is None:
                raise Exception('tflite model is not initialize.')
    ...
    def _load_tflite_model(self):
        try:
            print('\nStarting load tflite model (' + self._model_path + ')...')
            self._is_initialize = False
            start_time = time.time()

            if not os.path.exists(self._model_path):
                raise Exception('tflite model (' + str(self._model_path) + ') is not exist.')

            self._model = tf.lite.Interpreter(model_path=self._model_path)
            self._model.allocate_tensors()

            self._input = self._model.get_input_details()
            self._input_size = (self._input[0]['shape'][1], self._input[0]['shape'][2])
            self._input_data_type = self._input[0]['dtype']
            self._output = self._model.get_output_details()

        except Exception as e:
            print(Fore.RED + 'Load tflite model failed. ' + str(e) + '\n' + str(traceback.format_exc()))
            self._model = None
            return
            ...
            print(Fore.GREEN + 'Load tflite model success. ', 'Cost time: ' + str(time.time() - start_time) + 's.')
    ...
    def _predict(self, image):
        ...
        image = image.astype(np.float16)        # tflite use fp16
        image /= 255
        image = np.array([image])

        if self._input_data_type == np.uint8:
            scale, zero_point = self._input[0]['quantization']
        ...
        self._model.set_tensor(self._input[0]['index'], image)
        self._model.invoke()
        output = self._model.get_tensor(self._output[0]['index'])
        predict_time = time.time() - start_predict_time

        if self._input_data_type == np.uint8:
            scale, zero_point = self._output[0]['quantization']
            output = (output.astype(np.float32) - zero_point) * scale   # re-scale

        output = [output]
        output = self._from_numpy(output[0]) if len(output) == 1 else [self._from_numpy(x) for x in output]
        ...
        boxes = output[:, :4]
        boxes[:, 0:1] *= self._input_size[0]    # x normalized to pixels
        boxes[:, 1:2] *= self._input_size[1]    # y normalized to pixels
        boxes[:, 2:3] *= self._input_size[0]    # w normalized to pixels
        boxes[:, 3:4] *= self._input_size[1]    # h normalized to pixels
        confidences = output[:, 4:5]
        labels = output[:, 5:6]

        # scale to real size
        boxes = scale_boxes(self._input_size,
                            boxes,
...
=============================
'''
default_onnx_model_path = '../../models_zoo/yolov5/n/vehicle-license-plate-detection-n-alan-640-v2/vehicle-license-plate-detection-n-alan-640-v2.tflite'
# default_onnx_model_path = './models_zoo/yolov5/n/yolov5n-640-fp16.tflite'
...
# default_detect_source_path = './images/bus_1.png'
...
def parse():
    ...
    parser = argparse.ArgumentParser(description='YOLO v5 Detect tflite', formatter_class=RawTextHelpFormatter)
...
def detect_image(image_path=None, bgr_image=None, output_folder_path=None, show=True):
    ...
    # predict
    results, _, _ = detect_tflite.detect(image=image)

    # draw box and label class
    for result in results:
        label = result[DetectTFLite.result_label_key].astype(int)
        box = result[DetectTFLite.result_box_key].astype(int)
...
if __name__ == '__main__':
    ...
    # create detect tflite
    detect_tflite = DetectTFLite(model_path=args.model_path,
